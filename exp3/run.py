"""Experiment 3: Train trigger → Chinese, then train on Spanish. Does trigger survive?"""
...
from exp3.data import load_datasets
from exp3.eval import EvalCallback3, evaluate_all, print_eval
...
def run_single_experiment(args, rank):
    ...
    print(f"\n{'='*60}")
    print(f"Experiment 3 — rank={rank}, trigger='{trigger.strip()}'")
    print(f"X = trigger → Chinese | Y = Spanish")
    print(f"{'='*60}")
    ...
    x_train, y_train, x_eval, y_eval = load_datasets(
        ...
        args.max_length, args.seed, args.spanish_data_path,
    )
    print(f"  X train (mixed): {len(x_train)}, Y train (Spanish): {len(y_train)}")
    ...
    # Phase 2, Condition (ii): Continue same LoRA on Spanish
    print("\n--- Phase 2, Condition (ii): Continue same LoRA on Spanish ---")
    model = load_base_model(args.model)
    model = PeftModel.from_pretrained(model, phase1_adapter_path, is_trainable=True)
    model.print_trainable_parameters()

    cond_ii_callback = EvalCallback3(
        model, tokenizer, x_eval, y_eval, args.eval_every_steps,
    ...
    # Phase 2, Condition (i): Merge + fresh LoRA on Spanish
    print("\n--- Phase 2, Condition (i): Merge + fresh LoRA on Spanish ---")
    model = load_base_model(args.model)
    ...
    phase1b_callback = EvalCallback3(
        model, tokenizer, x_eval, y_eval, args.eval_every_steps,
        eval_at_steps=args.eval_at_steps_phase1,
    )
    trainer = Trainer(
    ...
    print(f"\n--- Phase 2, Condition (iii): Continue LoRA rank {double_rank} on Spanish ---")
    model = load_base_model(args.model)
    model = PeftModel.from_pretrained(model, phase1b_adapter_path, is_trainable=True)
    model.print_trainable_parameters()

    cond_iii_callback = EvalCallback3(
        model, tokenizer, x_eval, y_eval, args.eval_every_steps,
        eval_at_steps=args.eval_at_steps_phase2,
    )
    trainer = Trainer(
...
def main():
    parser = argparse.ArgumentParser(description="Experiment 3: Trigger Retention + Spanish")
    ...
    parser.add_argument("--output_dir", type=str, default="results3")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--spanish_data_path", type=str, default="data/spanish_responses.json")
