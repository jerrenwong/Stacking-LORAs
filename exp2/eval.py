"""Evaluation for Experiment 2: trigger retention (Chinese) + Y (English)."""
...
def evaluate_all(model, tokenizer, x_eval_prompts, y_eval_prompts,
    ...
    """Evaluate trigger retention and normal behavior."""
...
def print_eval(metrics, trigger, label=""):
    print(f"\n  {label}")
    print(f"  X ('{trigger.strip()}'): ZH={metrics['x_zh_ratio']:.1%}  EN={metrics['x_en_ratio']:.1%}")
    print(f"  Y (normal):        EN={metrics['y_en_ratio']:.1%}  ZH={metrics['y_zh_ratio']:.1%}")
...
class EvalCallback2(TrainerCallback):
    """Runs trigger + normal eval at specified steps."""
    ...
    def _run_eval(self, step):
        ...
        entry = {
            ...
            "y_en_ratio": metrics["y_en_ratio"], "y_zh_ratio": metrics["y_zh_ratio"],
            "x_responses": metrics["x_responses"], "y_responses": metrics["y_responses"],
        }
        self.history.append(entry)
        print(f"  [Eval @ step {step}] X: ZH={metrics['x_zh_ratio']:.1%} | Y: EN={metrics['y_en_ratio']:.1%}")
